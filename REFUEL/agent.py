from network import REFUEL, init_weights
import torch
import random
import pickle
import math
...
class Agent(object):
    ...
    def build_model(self, mode = 'normal'):
        self.current_REFUEL = REFUEL(input_size= self.num_slot, output_size=self.num_slot + self.num_disease, rebulid_size=self.num_slot)
        self.target_REFUEL = REFUEL(input_size= self.num_slot, output_size=self.num_slot + self.num_disease, rebulid_size=self.num_slot)
    ...
    def train(self, simulate_epoch_number, wandb):
        ...
        for epoch in range(simulate_epoch_number):
            ...
            writer.add_scalar('avg_recall', self.avg_recall, global_step=epoch)
            writer.add_scalar('avg_out', self.avg_out, global_step=epoch)
            ...
            writer.add_scalar('avg_recall_test', self.avg_recall_test, global_step=epoch)
            writer.add_scalar('avg_outs_test', self.avg_outs_test, global_step=epoch)
        self.save(self.path, epoch)    
