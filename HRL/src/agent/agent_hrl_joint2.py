...
from src.agent.agent_dqn import AgentDQN as LowerAgent
from src.policy_learning.dqn_torch import DQN, DQN2
from src.agent.utils import state_to_representation_last, reduced_state_to_representation_last
from src import dialogue_configuration
...
class AgentHRL_joint2(object):
    def __init__(self,slot_set, disease_symptom, parameter):
        ...
        if self.parameter.get("data_type")=='simulated':
            ...
            self.input_size_dqn_all = {1:81,2:81,3:81} # DXY
        elif self.parameter.get("data_type") == 'real':
            self.input_size_dqn_all = { 1:81,2:81,3:81,4:81}
        else:
            raise ValueError
        ...
        #for model in dirs:
            ...
            path_list = parameter['saved_model_path'].split('/')
            path_list.insert(-1, 'lower')
            path_list.insert(-1, str(label))
            temp_parameter[label]['saved_model_path'] = '/'.join(path_list)
        ...
        if parameter.get("train_mode") is False:
            ...
            self.master.restore_model(parameter.get("saved_model_path"))
    ...
    def train_dqn(self):
        ...
        # print(batch_size)
        if self.parameter['save_experience_pool'] and self.count < 15 :
            pickle.dump(file=open(self.parameter['experience_path'] + str(index) + '.json', "wb"), obj=self.experience_replay_pool)
            ...
            for group_id, lower_agent in self.id2lowerAgent.items():
                ...
                if len(lower_agent.experience_replay_pool) > 150:
                    pickle.dump(file=open(self.parameter['experience_path'] + str(group_id) + '/' +str(index) + '.json', "wb"), obj=lower_agent.experience_replay_pool)
