...
sys.path.append(os.getcwd().replace("src/dialogue_manager",""))

from src.state_tracker import StateTracker as StateTracker
from src import dialogue_configuration
from src.disease_classifier import dl_classifier
import numpy as np
...
class DialogueManager_HRL(object):
    ...
    def next(self, greedy_strategy, save_record, index, mode = 'train'):
        ...
        if self.parameter.get("disease_as_action")==False:
            ...
            if self.parameter.get("train_mode") == True:
                ...
                condition = state['turn']==self.parameter.get("max_turn")+16
            if (action_type == "disease" or condition) and self.parameter.get("agent_id") != 'agentdqn':# or lower_action in self.lower_action_history:
                #once the action is repeated or the dialogue reach the max turn, then the classifier will output the predicted disease

                disease = self.state_tracker.user.goal["disease_tag"]
                #Ys, pre_disease = self.model.predict([state_rep])
                pre_disease = [master_action_index - len(self.master_action_space)]
                '''
                if index>20:
                    print(Ys)
                    temp = self.exp_transform(Ys.detach().cpu().numpy()[0])
    ...
    def train_deep_learning_classifier(self, index, epochs):
        if self.parameter['save_experience_pool'] and index  < 15:
            pickle.dump(file=open(self.parameter['experience_path'] + 'disease_classifier' + '/' +str(index) + '.json', "wb"), obj=self.disease_replay['train'])
